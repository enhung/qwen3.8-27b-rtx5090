...
**當前狀態：**P1 已在 GPUtw 117 GB RTX 5090 完成 container 驗證；P3 53 GB RTX 5090 也通過至少 17 分鐘 cold start、SSH、10/10 container auth、Agent tool loop、80K/126K prompt、雙併發與正式並發 benchmark。8000 port 為 `public`，外部 GPUtw proxy 的 10-case authenticated path 已由 Mac client 全數通過；Hermes 受信外部 streaming chat、tool loop、續接與 Thinking-on 測試也已由操作端確認成功。Production 決策為 **GO**，profile 為 `mtp16`，Thinking off 預設但可按 request 開啟。先前重複輸出記錄為已恢復的暫時性 client/provider stream 異常，根因未確定。完整證據見 `evidence/2026-09-13-gputw-p1-live-gate.txt` 與 benchmark report。
