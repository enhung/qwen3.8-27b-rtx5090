...
| Hermes streaming / tool loop | pass；普通問答、tool call → tool result → final answer、續接均成功 | 外部 Hermes client；Thinking off 預設 |
| Production decision | **GO** | `mtp16` profile；Thinking off 預設；Thinking on parser 相容性列為後續改善 |
