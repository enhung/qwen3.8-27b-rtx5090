...
**P0 基線文件化與 P1 auth 候選程式已完成；production NO-GO。** 尚未 build/push P1 image，也未改 GPUtw live instance。這是刻意分開 P1 與後續 cache / host / lifecycle 變數；本次環境沒有 GPUtw key、Docker 或 5090，不能將本地測試當成 live 驗收。
...
- 新增只在 P1 功能分支 push 時建置的 GitHub Actions workflow；以 repo `GITHUB_TOKEN` 推送獨立 tag，避免依賴本機 Docker。尚未觸發。
...
已從公開 GHCR 查得 v2 manifest digest 並固定在 P1 Dockerfile。此任務環境的 shell、互動式 zsh 與 launchd 均沒有 `GPUTW_API_KEY`，也沒有 Docker；GitHub CLI 未登入，Computer Use 無法接管 Terminal，Chrome 操作連續逾時。尚缺：P1 image build/push digest、GPUtw live 外部 auth matrix、Hermes 真實 tool loop、long context、啟動與費用證據。安全路徑 allowlist 只根據 handoff 的 Chat Completions 行為；若 Hermes 走 `/v1/responses` 等路徑，必須用真實 trace 證明後再增加。Handoff 的價格、效能與 RAM 是既有記錄，本次未重測。

## 下一個可執行關卡

1. 依 `P1_DEPLOYMENT_RUNBOOK.md`，在有 Docker/registry 權限的環境 build `Dockerfile.gputw-auth`，記錄新 image digest，使用獨立 tag；不要覆蓋 v2。
2. 用原 117 GB host / 同一 pinned model、同一 Light flags 啟動 P1 instance；確認 `/vault` key file 存在但不讀出內容到 log。
3. 執行 `SECURITY.md` 的外部 auth matrix 與 `HERMES_SETUP.md` 的 tool loop；只有全通過才切 Hermes endpoint。
4. 然後才進入 P2 cache 路徑發現與 cold/warm 測試，再做 53 GB host 資格與 P4 on-demand。每階段保留 raw evidence 與 v2 rollback。
